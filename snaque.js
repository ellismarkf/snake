...
Snaque.prototype._prevTailIdx = function Snaque$_prevTailIdx() {
  // let nextLen = this._length - 2;
  // return nextLen < -1
  //   ? 0
  //   : (this._head + nextLen) & (this._capacity - 1);
  return (this._head + this._length - 2) & (this._capacity - 1);
}
...
Snaque.prototype.push = function Snaque$push(coords) {
...
Snaque.prototype.shift = function Snaque$shift() {
  ...
  let head = this._head;
  let ret = [this._x[head], this._y[head]];
  this._x[head] = -1;
  this._y[head] = -1;
...
Snaque.prototype.unshift = function Snaque$unshift(coords) {
  let capacity = this._capacity;
...
Snaque.prototype.forEach = function Snaque$forEach(fn) {
  for (let n = 0; n < this._length; n++) {
    ...
    fn.call(this, [this._x[i], this._y[i]], n, i, this);
  }
}
...
Snaque.prototype.every = function Snaque$every(fn) {
  ...
  for (let n = 0; n < this._length; n++) {
    let i = this._getIdx(n);
    let val = fn.call(this, [this._x[i], this._y[i]], n, i, this);
    if (!val) {
      pass = false;
      break;
    }
...
Snaque.prototype.sliceFrom = function Snaque$sliceFrom(begin) {
  ...
  for (let n = 0; n < length; n++) {
    let i = this._getIdx(n);
    if (i >= begin) {
      sliced.push([this._x[i], this._y[i]]);
      this._x[i] = -1;
      this._y[i] = -1;
    }
  }
...
Snaque.prototype.peekTail = function Snaque$peekTail() {
  ...
  return [this._x[this._tail], this._y[this._tail]];
};

Snaque.prototype.peekHead = function Snaque$peekHead() {
  if (this._length === 0) {
    console.log('peekHead returning invalid')
    return [-1, -1];
  }
  return [this._x[this._head], this._y[this._head]];
};
